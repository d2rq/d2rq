...
import org.d2rq.engine.MapFixture;
...
public class ExprTransformTest2 extends TestCase {
	...
	{
		ColumnName qualified = ColumnName.create(null, null, 
				Identifier.create(true, tableName),
				Identifier.create(true, attributeName));
		for (int i = 0; i < relation.length; i++) {
			NodeRelation rel = relation[i];
			for (ProjectionSpec p: rel.getBaseTabular().projections()) {
				Attribute attribute = ((AttributeProjectionSpec) p).getColumn();
				if (attribute.equals(qualified))
					return rel;
			}
		}
	...
	public void testDisjunction()
	{
		...
		Expression e1 = nm.getValueMaker().valueExpression("1");
		Expression e2 = nm.getValueMaker().valueExpression("2");
	...
	public void testSameTerm()
	{
		...
		TypedNodeMaker nm = (TypedNodeMaker) intvalue.nodeMaker(Var.alloc("o"));
		Expression expected = nm.getValueMaker().valueExpression("1");
