...
public class D2RServer {
	...
	public String graphURLDescribingResource(String resourceURI) {
		...
		try {
			return this.baseURI() + D2RServer.SPARQL_SERVICE_NAME + "?query="
	...
	public void start() {
		...
		if (config.isLocalMappingFile())
			this.dataset = new AutoReloadableDataset(
					config.getLocalMappingFilename(), true,
					overrideUseAllOptimizations, this);
		else
			this.dataset = new AutoReloadableDataset(config.getMappingURL(),
					false, overrideUseAllOptimizations, this);
	...
	/**
	 * Holds a shared {@link MetadataCreator} instance. This instance holds the
	 * meta data template defined in the configuration, caching this instance
	 * avoids reading the template on every request to a resource. The instance
	 * is constructed in the start() method.
	 * 
	 * @return the cached meta data creator instance
	 */
