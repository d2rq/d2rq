 ...
 * @version $Id: TranslationTable.java,v 1.9.1 2011/07/20 19:45:00 zazi Exp $
 */
public class TranslationTable extends MapObject
{
	 ...
	 * 
	 * @param dbValue
	 *            the value on the database side (usually coming from a DB
	 *            column)
	 * @param rdfValue
	...
	/**
	 * Sets a translation class. The translation class must implement the
	 * {@link Translator} interface. This method will take care of generating an
	 * instance of the class.
	 * 
	 * @param className
	 *            name of a class implementing {@link Translator}
	 */
	...
	public Translator translator()
	{
		...
		if (this.href != null)
		{
			return new TableTranslator(new TranslationTableParser(href)
					.parseTranslations());
		}
	...
	public void validate() throws D2RQException
	{
		if (!this.translations.isEmpty() && this.javaClass != null)
		{
			throw new D2RQException(
					"Can't combine d2rq:translation and d2rq:javaClass on "
							+ this,
		...
		{
			throw new D2RQException(
					"Can't combine d2rq:translation and d2rq:href on " + this,
					D2RQException.TRANSLATIONTABLE_TRANSLATION_AND_HREF);
		}
		if (this.href != null && this.javaClass != null)
		{
			throw new D2RQException(
					"Can't combine d2rq:href and d2rq:javaClass on " + this,
	...
	private Translator instantiateJavaClass()
	{
		try
		{
			...
			{
				throw new D2RQException("d2rq:javaClass " + this.javaClass
						+ " must implement " + Translator.class.getName());
			}
			...
			throw new D2RQException(
					"No suitable public constructor found on d2rq:javaClass "
							+ this.javaClass);
		} catch (ClassNotFoundException e)
		{
			throw new D2RQException("d2rq:javaClass not on classpath: "
					+ this.javaClass);
		}
	}
	...
	private Translator invokeConstructorWithArg(Class aClass, Resource r)
	{
		try
		{
			Constructor c = aClass
					.getConstructor(new Class[] { Resource.class });
	...
	public static class Translation
	{
		...
		{
			if (!(otherObject instanceof Translation))
				return false;
	...
	private class TableTranslator implements Translator
	{
		...
		TableTranslator(Collection translations)
		{
			...
			while (it.hasNext())
			{
				...
				this.translationsByDBValue
						.put(translation.dbValue, translation);
				this.translationsByRDFValue.put(translation.rdfValue,
						translation);
			}
		}
		...
		{
			Translation translation = (Translation) this.translationsByRDFValue
					.get(rdfValue);
		...
		{
			Translation translation = (Translation) this.translationsByDBValue
					.get(dbValue);
