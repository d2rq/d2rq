...
/**
 * TODO Move all engine-specific code from ConnectedDB to this interface and its
 * implementing classes
 * 
 * @author Richard Cyganiak (richard@cyganiak.de)
 * @author kurtjx (http://github.com/kurtjx)
 * @version $Id: ConnectedDB.java,v 1.37.1 2011/07/20 20:33:17 zazi
 *          (http://github.com/zazi) Exp $
 */
public class ConnectedDB
{
	...
	public static final String KEEP_ALIVE_PROPERTY = "keepAlive"; // interval
	// property,
	// value in
	// seconds
	public static final int DEFAULT_KEEP_ALIVE_INTERVAL = 60 * 60; // hourly
	public static final String KEEP_ALIVE_QUERY_PROPERTY = "keepAliveQuery"; // override
	// default
	// keep
	// alive
	// query
	public static final String DEFAULT_KEEP_ALIVE_QUERY = "SELECT 1"; // may not
	// work
	// for
	// some
	// DBMS
	 ...
	 */
	private static final String[] POSTGRESQL_IGNORED_SCHEMAS = {
			"information_schema", "pg_catalog" };
	private static final String[] ORACLE_IGNORED_SCHEMAS = { "CTXSYS",
			"EXFSYS", "FLOWS_030000", "MDSYS", "OLAPSYS", "ORDSYS", "SYS",
			"SYSTEM", "WKSYS", "WK_TEST", "WMSYS", "XDB" };
	private static final String[] MSSQL_IGNORED_SCHEMAS = { "sys",
			"INFORMATION_SCHEMA" };
	...
	// Lazy initialization for these two -- use the getSyntax() and dbType() for
	// access!
	...
	private Map uniqueIndexCache = new HashMap(); // RelationName => String =>
	// List of Strings
	...
	private class KeepAliveAgent extends Thread
	{
		...
		public void run()
		{
			...
			while (!shutdown)
			{
				...
				{ // may throw D2RQException at runtime
					log.error("Keep alive connection test failed: "
							+ e.getMessage());
				} finally
				{
					if (s != null)
	...
	{
		this(jdbcURL, username, password, true, Collections.EMPTY_SET,
				Collections.EMPTY_SET, Collections.EMPTY_SET,
				Collections.EMPTY_SET, Database.NO_LIMIT,
				Database.NO_FETCH_SIZE, null);
	}

	public ConnectedDB(String jdbcURL, String username, String password,
			boolean allowDistinct, Set textColumns, Set numericColumns,
			Set dateColumns, Set timestampColumns, int limit, int fetchSize,
			Properties connectionProperties)
	{
		...
		if (connectionProperties != null
				&& connectionProperties.containsKey(KEEP_ALIVE_PROPERTY))
		{
			int interval = DEFAULT_KEEP_ALIVE_INTERVAL;
			String query = DEFAULT_KEEP_ALIVE_QUERY;
			try
			{
				interval = new Integer((String) connectionProperties
						.get(KEEP_ALIVE_PROPERTY)).intValue();
				if (interval <= 0)
			...
			if (connectionProperties.containsKey(KEEP_ALIVE_QUERY_PROPERTY))
				query = connectionProperties
						.getProperty(KEEP_ALIVE_QUERY_PROPERTY);

			this.keepAliveAgent = new KeepAliveAgent(interval, query);
			this.keepAliveAgent.start();
			Runtime.getRuntime().addShutdownHook(shutdownHook);
			log.info("Keep alive agent is enabled (interval: " + interval
					+ " seconds, noop query: '" + query + "').");
		} else
	...
	private void connect()
	{
		...
		{
			throw new D2RQException("Database connection to " + jdbcURL
					+ " failed " + "(user: " + username + "): "
					+ ex.getMessage(), D2RQException.D2RQ_DB_CONNECTION_FAILED);
		}
		...
		try
		{
			...
			if (dbTypeIs(Oracle))
			{
				...
				{
					throw new D2RQException("Unable to set date format: "
							+ ex.getMessage(), D2RQException.D2RQ_SQLEXCEPTION);
				} finally
				{
					stmt.close();
				}
		...
		{
			throw new D2RQException("Database initialization failed: "
					+ ex.getMessage(), D2RQException.D2RQ_DB_CONNECTION_FAILED);
		}
	}
	...
	{
		Properties result = (connectionProperties == null) ? new Properties()
		...
		/*
		 * Enable cursor support in MySQL Note that the implementation is buggy
		 * in early MySQL 5.0 releases such as 5.0.27, which may lead to
		 * incorrect results. This is placed here as a later change requires a
		 * call to setClientInfo, which is only available from Java 6 on
		 */
	 ...
	 * 
	 * @return Node columnType D2RQ.textColumn or D2RQ.numericColumn or
	 *         D2RQ.dateColumn
	 */
	public int columnType(Attribute column)
	{
		...
		switch (type.typeId())
		{
		// TODO There are a bunch of others, see
		// http://java.sun.com/j2se/1.5.0/docs/api/java/sql/Types.html
		case Types.CHAR:
			return TEXT_COLUMN;
		case Types.VARCHAR:
			return TEXT_COLUMN;
		case ConnectedDB.SQL_TYPE_NVARCHAR:
			return TEXT_COLUMN;
		...
		case Types.TIMESTAMP:
			...
			// added uuid handling patch (created by kurtjx
			// (http://github.com/kurtjx)
		default:
			if ("NVARCHAR2".equals(type.typeName()))
				...
				throw new D2RQException("Unsupported database type code ("
						+ type + ") for column " + column.qualifiedName()
	 ...
	 * 
	 * <p>
	 * Assuming <tt>v1</tt> is a value from column1, and <tt>v2</tt> a value
	 ...
	 * compatible because equal values will be formatted in the same way when
	 * <tt>SELECT</tt>ed, e.g. <tt>1 = 1</tt>. But if one of them is
	 ...
	 * 
	 * <p>
	 * This is used by the engine when removing unnecessary joins. If two
	 * columns have compatible formatting, then we can sometimes use one in
	 * place of the other when they are known to have equal values. But not if
	 * they are incompatible, because e.g. <tt>http://example.org/id/1</tt> is
	 * different from <tt>http://example.org/id/0000000001</tt>.
	 * </p>
	 * 
	...
	private boolean isZerofillColumn(Attribute column)
	{
		...
		{
			zerofillCache.put(column, new Boolean(schemaInspector()
					.isZerofillColumn(column)));
		}
	...
	{
		if (!uniqueIndexCache.containsKey(tableName)
				&& schemaInspector() != null)
			uniqueIndexCache.put(tableName, schemaInspector().uniqueColumns(
					tableName));
		return (HashMap) uniqueIndexCache.get(tableName);
	}

	private final static Pattern singleQuoteEscapePattern = Pattern
			.compile("([\\\\'])");
	private final static Pattern singleQuoteEscapePatternOracle = Pattern
			.compile("(')");

	/**
	 * Wraps s in single quotes and escapes special characters to avoid SQL
	 * injection
	 */
	public String singleQuote(String s)
	{
		...
		{
			return "'"
					+ singleQuoteEscapePatternOracle.matcher(s).replaceAll(
							"$1$1") + "'";
		}
		return "'" + singleQuoteEscapePattern.matcher(s).replaceAll("$1$1")
				+ "'";
	}
	...
	/**
	 * Some Databases do not handle large entries correctly. For example
	 * MSAccess cuts strings larger than 256 bytes when queried with the
	 * DISTINCT keyword. TODO We would need some assertions about a database or
	 * specific columns.
	 */
	...
	/**
	 * In some situations, MySQL stores table names using lowercase only, and
	 * then performs case-insensitive comparison. We need to account for this
	 * when comparing table names reported by MySQL and those from the mapping.
	 * 
	 * @see <a
	 *      href="http://dev.mysql.com/doc/refman/5.0/en/identifier-case-sensitivity.html">MySQL
	 *      Manual, Identifier Case Sensitivity</a>
	 */
