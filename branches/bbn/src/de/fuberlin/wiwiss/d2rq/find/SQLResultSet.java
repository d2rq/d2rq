...
/**
 * Contains the result set from one SQL query and transforms it into triples. A
 * triple is produced for TripleMaker in TripleMaker and each row in the result
 * set.
 * 
 * <p>
 * History:<br>
...
public class SQLResultSet {
	...
	public void close() {
		...
		try {
			...
			if(this.resultSet.getStatement() != null){
	...
	/**
	 * Gets the current row from the result set in an array which is passed to
	 * the triple makers
	 */
	protected String[] nextRow() {
		...
		try {
			...
			logger.debug( "Row Count: " + InfoD2RQ.totalNumberOfReturnedRows );
			logger.debug( "Column Count: " + InfoD2RQ.totalNumberOfReturnedFields );
			ex.printStackTrace();
			logger.error(ex.getMessage());
			return null;
		}
