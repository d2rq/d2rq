 ...
 */
public class GraphD2RQ extends GraphBase {
...
//		this.style = ReificationStyle.Minimal;
	...
	public ExtendedIterator graphBaseFind( TripleMatch m ) {
		...
		QueryCombiner combiner = new QueryCombiner( relationships );
	...
	public ArrayList propertyBridgesForTriple(Triple t, ArrayList relationships ) { // PropertyBridge[]
		QueryContext context = new QueryContext();	
		Iterator it = this.propertyBridges.iterator();
		...
		while (it.hasNext()) {
            ...
            if( !bridgeDoesFit( bridge, relationships, t) ){
                continue;
            }
			if (Logger.instance().debugEnabled()) {
				Logger.instance().debug("--------------------------------------------");
	...
	/**
	 * Determines if the property bridge in question does actually fit 
	 * the triple in the query.  If it does the it returns true.  This is 
	 * to ensure that property bridges that are similar aren't confused when
	 * making a sql query.   
	 * @param bridge the bridge being evaluated.
	 * @param relationships The relationships associated with the triple that 
	 * we are trying to pair a property bridge with.
	 * @param t the triple statement in the query. 
	 * @return true if it does fit, else false. 
	 */
	
    private boolean bridgeDoesFit( PropertyBridge bridge, ArrayList relationships, Triple t ){
        Iterator iter = bridge.getSubjectMaker().getColumns().iterator();
        boolean retVal = false;
        while( iter.hasNext() ){
            Column col = (Column)iter.next();
            String bridgeTable = col.getTableName();
            for( int i = 0;i < relationships.size();i++ ){
                TripleRelationship tr = (TripleRelationship)relationships.get(i); 
                String table = tr.getTable();
                //This Optimization cannot work without this table being specified to return true and try to 
                //run the query for all types of this property. 
                if( table == null ){
                    return true;
                }
                if( table.equals( bridgeTable ) ){
                    bridge.setBoundVar( t.getSubject().toString() );
                    retVal = true;
                }
            }
        }
     ...
     * @return The Jena model associted with the data mapping. 
     */
